...
def SaveToNetCDF4(outfilename, predicted, netCDFData, timeStamp):
    ...
    times.units = 'seconds since 1970-01-01 00:00:00'  # "seconds since 1800-01-01 00:00:00"
    # pdb.set_trace()
    ...
    # Convert to rainfall
    lwe = ncfile.createVariable('lwe_precipitation_rate', np.float32, ('time', 'Y', 'X'), fill_value=9.9621e+36)
    # estimated precipitation from predicted Reflectivity
    lwe[0, :, :] = convertToPr(predicted)

    lwe.units = netCDFData.variables['lwe_precipitation_rate'].attrs['units']
    lwe.long_name = netCDFData.variables['lwe_precipitation_rate'].attrs['long_name']
    lwe.standard_name = netCDFData.variables['lwe_precipitation_rate'].attrs['standard_name']
    lwe.coordinates = 'lon lat'
    lwe.grid_mapping = netCDFData.variables['lwe_precipitation_rate'].attrs['grid_mapping']
...
def testMosaicTiledProductSingleTimePrediction(t, httpLinkOrFile):
    ...
    model = loadMLModel(
        '/lustre/storeB/project/IT/geout/weamyl/weamyl_model/ml_model/ML_usage_codesample/128x128_trained_135ep_20210107.hdf5')

    # make prediction
    # End of code relevant to how we use the model for prediction
    predicted_Reflectivity_At_t5 = predictMosaic(model, actual_Reflectivity_At_t0)
    # pdb.set_trace()
    ...
    outfilename = '/lustre/storeB/project/IT/geout/weamyl/weamyl_model/output/predicted_reflectivity_yrwms-nordic_latest_5min_test.nc'
    print('Saving data to file:', outfilename)

    if sys.argv[1] == 'latest':
        timeStampPoz2 = 0
    else:
        timeStampPoz2 = (int(t.hour) * 60 + int(t.minute)) // aquisitionIntervall + 1
...
def testOne2OneSingleProductSingleTimePrediction(year, month, day, hour, minute):
    ...
    # Begin of code relevant to how we use the model for prediction
    # Load machine learning model (load once can be used for multiple predictions)
    model = loadMLModel('./ML_usage_codesample/700ep_trained_with_20210107.hdf5')
    ...
    outfilename = "predicted_reflectivity_yrwms-nordic_mos_pcappi-0-dbz.noclass-clfilter-novpr-clcorr-block_nordiclcc-1000_" + year + month + day + "T" + hour + str(
        (int(minute) + 5)) + ".nc"
