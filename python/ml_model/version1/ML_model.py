...
def SaveToNetCDF4(outfilename, predicted, netCDFData, timeStamp):
    ...
    ncfile.history = 'Running script received from UBB Romania - model version 1.0.0'
...
def testMosaicTiledProductSingleTimePrediction(t, httpLinkOrFile0, httpLinkOrFile1,httpLinkOrFile2, httpLinkOrFile3):
    ...
    aquisitionIntervall = 5;
    t_00 = t
    t5 = (t + datetime.timedelta(minutes=5))
    ...
    actual_Reflectivity_At_t_00 = loadReflectivityAt(netCDFData, t_00.hour, t_00.minute)
    actual_Reflectivity_At_t_05 = loadReflectivityAt(netCDFData1, t_05.hour, t_05.minute)
    actual_Reflectivity_At_t_10 = loadReflectivityAt(netCDFData2, t_10.hour, t_10.minute)
    actual_Reflectivity_At_t_15 = loadReflectivityAt(netCDFData3, t_15.hour, t_15.minute)
    # e.g. load reflectivity at 14:35  1.8.2021
    # actual_Reflectivity_At_t5 = loadReflectivityAt(netCDFData,t.hour,str(int(t.minute) + 5))
    #pdb.set_trace()
    input4 = [actual_Reflectivity_At_t_00,
              actual_Reflectivity_At_t_05,
              actual_Reflectivity_At_t_10,
              actual_Reflectivity_At_t_15]
    ...
    predicted_Reflectivity_At_t5 = predictMosaic(model, input4)
    # pdb.set_trace()
    timeStamp = (t + datetime.timedelta(minutes=5))#.strftime("%Y%m%dT%H%M%SZ")
    #print(timeStamp)
    # outfilename = '/lustre/storeB/project/IT/geout/weamyl/weamyl_model/output/predicted_' + 'reflectivity_yrwms-nordic_mos_pcappi-0-dbz_noclass-clfilter-novpr-clcorr-block_nordiclcc-1000_'+ timeStamp.strftime("%Y%m%dT%H%M%SZ") +".nc"
        
    if sys.argv[1] == 'latest':
        outfilename = '/lustre/storeB/project/IT/geout/weamyl/weamyl_model/output/version1/predicted_reflectivity_yrwms-nordic_latest_5min.nc'
    else:
        outfilename = '/lustre/storeB/project/IT/geout/weamyl/weamyl_model/output/version1/predicted_reflectivity_yrwms-nordic_'+ t.strftime("%Y%m%d") +'.nc'

    print('Saving data to file:', outfilename)
    SaveToNetCDF4(outfilename, predicted_Reflectivity_At_t5, netCDFData, timeStamp)
    
    print('Keeping intermediate files ...')
    if sys.argv[1] == 'latest':
       outfilename = '/lustre/storeB/project/IT/geout/weamyl/weamyl_model/output/os.environ.get("USER")_predicted_reflectivity_yrwms-nordic_'+ t5.strftime("%Y%m%dT%H%M%SZ") +'_version1.nc'
       print('Saving data to file:', outfilename)
       SaveToNetCDF4(outfilename, predicted_Reflectivity_At_t5, netCDFData, timeStamp)
...
if sys.argv[1] == 'latest':
    ...
    httpLinkOrFile0 = max(list_of_files, key=os.path.getctime)
    timestamp = httpLinkOrFile0[-19:-3]
    ...
    #pdb.set_trace()
    #print(t)
    httpLinkOrFile1 = "/lustre/storeB/project/remotesensing/radar/reflectivity-nordic/latest/yrwms-nordic.mos.pcappi-0-dbz.noclass-clfilter-novpr-clcorr-block.nordiclcc-1000." + t_05.strftime("%Y%m%dT%H%M") + "00Z.nc"
    httpLinkOrFile2 = "/lustre/storeB/project/remotesensing/radar/reflectivity-nordic/latest/yrwms-nordic.mos.pcappi-0-dbz.noclass-clfilter-novpr-clcorr-block.nordiclcc-1000." + t_10.strftime("%Y%m%dT%H%M") + "00Z.nc"
    httpLinkOrFile3 = "/lustre/storeB/project/remotesensing/radar/reflectivity-nordic/latest/yrwms-nordic.mos.pcappi-0-dbz.noclass-clfilter-novpr-clcorr-block.nordiclcc-1000." + t_15.strftime("%Y%m%dT%H%M") + "00Z.nc"

else:
    arg = sys.argv # e.g. 20220406
    timestamp = arg[1]
    t = datetime.datetime(int(timestamp[0:4]), int(timestamp[4:6]), int(timestamp[6:8]))
    t_00 = t
    t_05 = (t - datetime.timedelta(minutes=5))  # .strftime("%Y%m%dT%H%M%SZ")
    t_10 = (t - datetime.timedelta(minutes=10))  # .strftime("%Y%m%dT%H%M%SZ")
    t_15 = (t - datetime.timedelta(minutes=15))  # .strftime("%Y%m%dT%H%M%SZ") 
    httpLinkOrFile0 = "https://thredds.met.no/thredds/fileServer/remotesensing/reflectivity-nordic/" +  timestamp[0:4] + '/' + timestamp[4:6] + "/yrwms-nordic.mos.pcappi-0-dbz.noclass-clfilter-novpr-clcorr-block.nordiclcc-1000." + timestamp + ".nc"
    httpLinkOrFile1 = "https://thredds.met.no/thredds/fileServer/remotesensing/reflectivity-nordic/" +  str(t_05.year) + '/' + f"{t_05.month:02}" + "/yrwms-nordic.mos.pcappi-0-dbz.noclass-clfilter-novpr-clcorr-block.nordiclcc-1000." + t_05.strftime("%Y%m%d") + ".nc"
    httpLinkOrFile2 = "https://thredds.met.no/thredds/fileServer/remotesensing/reflectivity-nordic/" +  str(t_10.year) + '/' + f"{t_10.month:02}" + "/yrwms-nordic.mos.pcappi-0-dbz.noclass-clfilter-novpr-clcorr-block.nordiclcc-1000." + t_10.strftime("%Y%m%d") + ".nc"
    httpLinkOrFile3 = "https://thredds.met.no/thredds/fileServer/remotesensing/reflectivity-nordic/" +  str(t_15.year) + '/' + f"{t_15.month:02}" + "/yrwms-nordic.mos.pcappi-0-dbz.noclass-clfilter-novpr-clcorr-block.nordiclcc-1000." + t_15.strftime("%Y%m%d") + ".nc"
